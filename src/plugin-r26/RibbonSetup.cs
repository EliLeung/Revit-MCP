...
namespace RvtMcp.Plugin
{
    ...
    public static class RibbonSetup
    {
        ...
        public static RibbonResult Create(UIControlledApplication application, RvtMcpConfig config = null, BakedToolRuntimeCache runtimeCache = null)
        {
            ...
            panel.AddItem(new PushButtonData("ManageConnections", "连接管理", assemblyPath,
                "RvtMcp.Plugin.Commands.ManageConnectionsCommand") {
                LargeImage = IconGenerator.Info32, Image = IconGenerator.Info16,
                ToolTip = "查看 MCP 客户端连接、添加备注、断开连接或临时阻止重连"
            });
