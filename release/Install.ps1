# Revit-MCP installer, Apache-2.0. Installs per user; does not edit MCP host configs.
...
if ($TestRoot) {
    ...
    $dataRoot = Join-Path $test 'LocalAppData\Revit-MCP'
    ...
    $dataRoot = Join-Path $env:LOCALAPPDATA 'Revit-MCP'
...
if ($Uninstall) {
    if (!$state -or !$state.active) { Write-Output 'Revit-MCP is not active. Nothing to restore.'; return }
...
$target = if ($registrations.Count -eq 1) { $registrations[0] } else { Join-Path $addins 'Revit-MCP.addin' }
if ($registrations.Count -eq 0 -and (Test-Path -LiteralPath $target)) { throw 'Revit-MCP.addin already belongs to another registration. No files were overwritten.' }
if ($state -and $state.active) {
    ...
    Write-Output "Revit-MCP $version is already installed."
    return
}
...
<Name>Revit-MCP</Name><Assembly>$assembly</Assembly>
<FullClassName>RvtMcp.Plugin.App</FullClassName><AddInId>{$pluginId}</AddInId>
<VendorId>RMCP</VendorId><VendorDescription>Revit-MCP; enhanced from bimwright/rvt-mcp (Apache-2.0)</VendorDescription>
</AddIn></RevitAddIns>
"@
...
if (!$PSCmdlet.ShouldProcess($target, "Install Revit-MCP $version and back up existing registration")) { return }
...
$exe = Join-Path $destination 'server\Revit-MCP.exe'
$commandLiteral = ConvertTo-Json -InputObject $exe -Compress
$snippet = "[mcp_servers.revit-mcp]`r`ncommand = " + $commandLiteral + "`r`nargs = [`"--toolsets`", `"all`"]`r`n"
...
Write-Output 'Installed. Open Revit 2026 > Add-Ins > Revit-MCP > Connection Manager.'
